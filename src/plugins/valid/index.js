...
function _getRegExp (f) {
    ...
    switch (f) {
        ...
        case 'letterStart': return new RegExp('^([a-zA-Z]([a-zA-Z\\d]){' + (parseInt(d) - 1) + ',' + (parseInt(c) - 1) + '})$');
        case 'length': return new RegExp('^(([a-zA-Z\\d]){' + d + ',' + c + '})$');
        case 'express': return new RegExp(d);
        default: return 'null';
    }
};
