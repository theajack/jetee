...
function _checkRouterName (k, route) {
    if (k === Jet.router.index) {
        Jet.router.indexMap = route;
    }
    _addIntoRouter(k, route.component);
        ...
        if (url._JT_has('http://') || url._JT_has('https://')) {
            ...
            if (file != undefined) {
                ...
                if (typeof JUI !== 'undefined') {
                    window.JUI.dialog.removeAll();
                }
                let html = C._buildCompHtml(file);
...
Jet.prototype.$route.back = function () {
    Jet.router.back();
};
Jet.prototype.$route.forward = function () {
    Jet.router.forward();
};
