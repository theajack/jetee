...
export default function (Jet) {
    ...
    var _cssConfCom = 'CssConfCommon';
    // var _COMMON_CSS = 'COMMON_CSS';
    var conf = {};
    ...
    function _loadCommonCssCall (commonCss, length) {
        if (length == 0) {
            if (commonCss.length > 0) {
                var res = C._less(conf._replaceCssVar(commonCss.join('')));
                if (Jet.__css_conf.conf.commonScoped) {
                    res = C._addScopedCss(res, 'RouterOut');
                }
    ...
    Jet.style = function (conf) {
        Jet.__css_conf.conf = conf;
    };
}